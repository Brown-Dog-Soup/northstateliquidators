...
public sealed class ItemsFunction
{
    ...
    public sealed record PatchRequest(
        ...
        string? notes,
        string? description,
        bool? isHighlight);   // B8: feature this item under the box on the website
        ...
        CancellationToken ct)
    {
        ...
        if (body.isHighlight.HasValue)       { sets.Add("is_highlight = @h");  p.Add("h", body.isHighlight.Value); }
...
SELECT id, manifest_id, lpn, upc, asin, qty, condition, title, description, brand, category,
       ...
       photo_blob_url, enrich_status, notes, created_at, is_highlight
