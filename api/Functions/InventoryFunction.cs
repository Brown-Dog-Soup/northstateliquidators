...
public sealed class InventoryFunction
{
        ...
        CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(lot))    { where.Add("(lot_id = @Lot OR source_pallet_ref = @Lot)"); p.Add("Lot", lot); }
...
SELECT TOP (@Limit)
    ...
    qty_in_manifest, lot_id, source_pallet_ref, imported_at,
...
SELECT TOP 30 source_pallet_ref AS lot, COUNT(*) AS n
FROM dbo.v_inventory
WHERE source_pallet_ref IS NOT NULL
GROUP BY source_pallet_ref
